...
master = mesh_input.data_master()

batch_size = 256
epoch_num_cnn = 5
keep_pro = 0.9

model = FusedModel.FusedModel(master.lookup_matrix)
...
with tf.Session() as sess:
    ...
    print('pretraining Transfer-CNN Part')
    for epoch in range(epoch_num_cnn):
        master.shuffle()
        for iter, (batch_x, batch_y) in enumerate(master.batch_iter(batch_size)):
            loss_fetch, output, _ = sess.run([model.loss_cnn_t, model.prediction_cnn_t, model.optimizer_cnn_t],
