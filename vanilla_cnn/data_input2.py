...
class data_master:
    def __init__(self):
        # with open('../PKL2/lookup_matrix.pkl', 'rb') as file:
        #     self.embeddings = pickle.load(file)

        self.embeddings = np.load('../PKL2/lookup_matrix.npy')
        with open('../PKL2/lookup_dict.pkl', 'rb') as file:
            self.word_dict = pickle.load(file)
        with open('../PKL2/code_dict.pkl', 'rb') as file:
            self.code_dict = pickle.load(file)
        with open('../PKL2/all_text.pkl', 'rb') as file:
            all_text = pickle.load(file)
        with open('../PKL2/all_code.pkl', 'rb') as file:
        ...
        if shuffle:
            data_sentences, data_labels = self.shuffle(
                data_sentences,
                data_labels)
    ...
    def shuffle(self, data_sentences=None, data_labels=None):
